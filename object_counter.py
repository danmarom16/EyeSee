...
#    self.classwise_counts[self.model.names[cls]] = {"CLEAN_IN": 0, "DIRTY_IN": 0, "CLEAN_OUT": 0, "DIRTY_OUT": 0}
    ...
    def store_classwise_counts(self, cls):
        if self.model.names[cls] not in self.classwise_counts:
            self.classwise_counts[self.model.names[cls]] = {CountType.CLEAN_IN: 0, CountType.DIRTY_IN: 0,
                                                            CountType.CLEAN_OUT: 0, CountType.DIRTY_OUT: 0}


    def count_client_dirty_exit(self, dwell_times, track_id, cls=0):

        dwell_times[track_id][DwellTime.EXIT] = self.video_manager.get_current_time()
        dwell_times[track_id][DwellTime.DWELL] = (dwell_times[track_id][DwellTime.EXIT] -
                                                  dwell_times[track_id][DwellTime.ENTRANCE])
        dwell_times[track_id][DwellTime.EXIT_TYPE] = ExitType.DIRTY

        self.dirty_out_count += 1
        self.classwise_counts[self.model.names[cls]][CountType.DIRTY_OUT] += 1

    def count_not_client_dirty_exit(self, track_id, dirty_ids, cls=0):
        dirty_ids.remove(track_id)
        self.classwise_counts[self.model.names[cls]][CountType.DIRTY_IN] -= 1
    ...
    def count_in(self, cls):
        ...
        if self.model.names[cls] not in self.classwise_counts:
            self.classwise_counts[self.model.names[cls]] = {CountType.CLEAN_IN: 0, CountType.DIRTY_IN: 0,
                                                            CountType.CLEAN_OUT: 0, CountType.DIRTY_OUT: 0}
            self.classwise_counts[self.model.names[cls]][CountType.CLEAN_IN] += 1
        self.store_classwise_counts(cls)

    def count_out(self, track_id, cls):
        self.out_count += 1
        self.classwise_counts[self.model.names[cls]][CountType.CLEAN_OUT] += 1
